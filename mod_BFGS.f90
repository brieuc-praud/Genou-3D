Module mod_BFGS
   ...
   Real(PR) :: eps = 1.e-2_PR !Seuil pour la norme du gradient
...
Contains
   Subroutine BFGS(u, theta)
      ...
      Do While ( norme2(grad) > eps)
         ...
         End Do
         B = B + (k/l)*Id - (m/n)*Id
         grad = y + grad !On évite ici de refaire appel à la fonction gradient()
      End Do
   End Subroutine BFGS
   ...
   Function Wolfe(x, d, grad, theta) Result(alpha)
      ...
      Do While (.NOT. critere_de_Wolfe )
         ...
         Else
            gradad = gradient(x + alpha*d, theta)
            gadd = 0._PR
            Do i=1, 5
               gadd = gradad(i)*d(i)
            End Do
   ...
   Function gradient(x, theta)
      Real(PR), Dimension(5), Intent(In) :: x
      Real(PR), Intent(In) :: theta
      Real(PR), Dimension(5) :: gradient

      Integer :: i

      Do i=1, 5
         gradient(i) = dvp(x, i, theta)
      End Do
